...
SURICATA_LOG_FILE = "logs/alerts-only.json"
...
def analyze():
    ...
    if df.empty:
        ...
        return
    print("Загружено событий:", len(df))
    ...
    # проверка VirusTotal
    if not vt_enabled():
        print("API ключ VirusTotal не найден. Проверка через VirusTotal пропущена.")
    else:
        print("Проверка IP-адресов через VirusTotal...")
    ...
    print("Отчёты успешно сохранены.")
    # строим график
    top5 = report_df.head(5)
    plt.figure(figsize=(8, 5))
    sns.barplot(x="ip", y="count", data=top5)
    plt.title("Топ-5 IP-адресов по количеству событий")
    plt.tight_layout()
